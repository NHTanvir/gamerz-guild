...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "its_plugin_{$_args['post_type']}", 'its_plugin' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "its_plugin_{$_args['post_type']}", $posts, 'its_plugin', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'its_plugin' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'its_plugin_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /gamerz-guild directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with gamerz-guild_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'its_plugin_template_overwrite_dir', get_stylesheet_directory() . '/gamerz-guild/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( ITS ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'gamerz-guild' );
		}
	}
}